...
def simpleVerify(state, constraint_data):
    ...
    for s in state:
        id += 1
        if s != "disabled":
            if (s < int(constraint_data[id_lookup(constraint_data, id)]["value_range_min"]) or s > int(constraint_data[id_lookup(constraint_data, id)]["value_range_max"])):
                return False
...
def simpleGenerate(initial_state, constraint_data, disable, max_depth):
    ...
    if state_depth > max_depth:
        if simpleVerify(initial_state, constraint_data) == False:
    ...
    max = int(constraint_data[id_lookup(constraint_data, state_depth)]["value_range_max"])
