                ...
                if (default_feature_args.get(id_lookup(constraint_data, id), None) == None):
                    ...
                    if (constraint_data[id_lookup(constraint_data, id)]["takes_value"] == "no"):
                        ...
                        if (constraint_data[id_lookup(constraint_data, id)]["value_range_max"] != None):
                            ...
                            temp.arg[id_lookup(constraint_data, id)] = constraint_data[id_lookup(constraint_data, id)][
                                "value_range_min"]
                            temp2.arg[id_lookup(constraint_data, id)] = constraint_data[id_lookup(constraint_data, id)][
                                "value_range_max"]
                            temp3.arg[id_lookup(constraint_data, id)] = nextPow2(
                                int(constraint_data[id_lookup(constraint_data, id)]["value_range_min"]) + 1)
                        else:
                            # case just min
                            temp.arg[id_lookup(constraint_data, id)] = constraint_data[id_lookup(constraint_data, id)][
                                "value_range_min"]
...
def ConfigToCMD(config, constraint_data, location):
    ...
    for arg in config.arg:
        ...
        if ((default_feature_args.get(arg, None) == None) or ((default_feature_args.get(arg, None) != None) and (
                str(default_feature_args[arg]) != str(config.arg[arg])))):
